...
import com.godsandtowers.billing.google.Security.VerifiedPurchase;

/**
 * This class sends messages to Android Market on behalf of the application by connecting (binding) to the
 * MarketBillingService. The application creates an instance of this class and invokes billing requests through this
 * service.
 * 
 * The {@link BillingReceiver} class starts this service to process commands that it receives from Android Market.
 * 
...
public class BillingService extends Service implements ServiceConnection {
	...
	/**
	 * The list of requests that we have sent to Android Market but for which we have not yet received a response code.
	 * The HashMap is indexed by the request Id that each request receives when it executes.
	 */
	...
	abstract class BillingRequest {
		...
		/**
		 * Called when a remote exception occurs while trying to execute the {@link #run()} method. The derived class
		 * can override this to execute exception-handling code.
		 * 
		 * @param e
		 *            the exception
		 */
	...
	public void handleCommand(Intent intent, int startId) {
		if (intent == null) {
			Log.i(TAG, "Received a null intent");
			return;
		}
	 ...
	 * {@link BillingReceiver} receives an intent with the action {@link Consts#ACTION_NOTIFY}. Returns false if there
	 ...
	 * which we include in our request. The server responds with the purchase information, encoded as a JSON string, and
	 * sends that to the {@link BillingReceiver} in an intent with the action
	 * {@link Consts#ACTION_PURCHASE_STATE_CHANGED}. Returns false if there was an error trying to connect to the
	 ...
	 * {@link ResponseHandler#purchaseResponse(Context, PurchaseState, String, String, long)} for each verified
	 * purchase.
	 * 
	 * @param startId
	 *            an identifier for the invocation instance of this service
	...
	/**
	 * This is called when we receive a response code from Android Market for a request that we made. This is used for
	 * reporting various errors and for acknowledging that an order was sent to the server. This is NOT used for any
	 * purchase state changes. All purchase state changes are received in the {@link BillingReceiver} and passed to this
	 * service, where they are handled in {@link #purchaseStateChanged(int, String, String)}.
	 * 
	 * @param requestId
	 *            a number that identifies a request, assigned at the time the request was made to Android Market
	 * @param responseCode
