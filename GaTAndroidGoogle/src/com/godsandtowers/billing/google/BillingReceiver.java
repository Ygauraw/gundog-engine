...
/**
 * This class implements the broadcast receiver for in-app billing. All asynchronous messages from Android Market come
 * to this app through this receiver. This class forwards all messages to the {@link BillingService}, which can start
 * background threads, if necessary, to process the messages. This class runs on the UI thread and must not do any
...
public class BillingReceiver extends BroadcastReceiver {
	 ...
	 * forwards the messages on to the {@link BillingService}, which handles the communication back to Android Market.
	 * The {@link BillingService} also reports state changes back to the application through the {@link ResponseHandler}
	 * .
	 */
	@Override
	public void onReceive(Context context, Intent intent) {
		...
		if (Consts.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
			...
			if (Consts.DEBUG) {
				Modules.LOG.info(TAG, "notifyId: " + notifyId);
			}
	 ...
	 * The request includes a nonce (random number used once) that we generate and Android Market signs and sends back
	 * to us with the purchase state and other transaction details. This BroadcastReceiver cannot bind to the
	 * MarketBillingService directly so it starts the {@link BillingService}, which does the actual work of sending the
	 * message.
	 * 
	 * @param context
	 *            the context
