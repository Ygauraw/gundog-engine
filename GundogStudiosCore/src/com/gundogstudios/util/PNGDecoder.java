...
package com.gundogstudios.util;
...
public class PNGDecoder {
	...
	public PNGDecoder(InputStream stream) {
		this();
		try {
			loadImage(stream);
		} catch (IOException e) {
	...
	/**
	 * @see org.newdawn.slick.opengl.ImageData#getHeights()
	 */
	...
	/**
	 * @see org.newdawn.slick.opengl.ImageData#getWidths()
	 */
	public int getWidth() {
		return width;
	}
	 ...
	 * @param stride
	 *            The image stride to read (i.e. the number of bytes to skip each line)
	 * @param flip
	 *            True if the PNG should be flipped
	 * @throws IOException
	 *             Indicates a failure to read the PNG either invalid data or not enough room in the buffer
	 */
	...
	/**
	 * Copy some data into the given byte buffer expanding the data based on indexing the palette
	 * 
	 * @param buffer
	...
	/**
	 * Copy the data given directly into the byte buffer (skipping the filter byte);
	 * 
	 * @param buffer
	 *            The buffer to write into
	 * @param curLine
	 ...
	 *            The line before
	 * @throws IOException
	 *             Indicates a failure to unfilter the data due to an unknown filter type
	 */
	...
	/**
	 * Sub unfilter {@url http://libpng.nigilist.ru/pub/png/spec/1.2/PNG-Filters.html}
	 * 
	 * @param curLine
	...
	/**
	 * Up unfilter {@url http://libpng.nigilist.ru/pub/png/spec/1.2/PNG-Filters.html}
	 * 
	 * @param prevLine
	...
	/**
	 * Average unfilter {@url http://libpng.nigilist.ru/pub/png/spec/1.2/PNG-Filters.html}
	 * 
	 * @param prevLine
	 *            The line of data read before the current
	 * @param curLine
	...
	/**
	 * Paeth unfilter {@url http://libpng.nigilist.ru/pub/png/spec/1.2/PNG-Filters.html}
	 * 
	 * @param prevLine
	 *            The line of data read before the current
	 * @param curLine
	 ...
	 * 
	 * @throws IOException
	 *             Indicates a failure to determine chunk information from the stream
	 */
	 ...
	 * @throws IOException
	 *             Indicate a failure to read data or a different chunk on the stream
	 */
	 ...
	 * @throws IOException
	 *             Indicate a failure to read the appropriate data from the chunk
	 */
	 ...
	 * @throws IOException
	 *             Indicates there is no more data left or invalid data has been found on the stream.
	 */
	...
	public ByteBuffer loadImage(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent)
		...
		scratch = ByteBuffer.allocateDirect(texWidth * texHeight * perPixel).order(ByteOrder.nativeOrder());
		...
		if (!hasAlpha() && forceAlpha) {
			ByteBuffer temp = ByteBuffer.allocateDirect(texWidth * texHeight * 4).order(ByteOrder.nativeOrder());
			for (int x = 0; x < texWidth; x++) {
				for (int y = 0; y < texHeight; y++) {
	...
	public String toString() {
		return "PNGImageData [width=" + width + ", height=" + height + ", colorType=" + colorType + ", bytesPerPixel="
				+ bytesPerPixel + ",  bitDepth=" + bitDepth + ", texWidth=" + texWidth + ", texHeight=" + texHeight
				+ "]";
	}
