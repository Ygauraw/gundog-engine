...
class Stripifier {
		...
		for (int i = 0; i < numTriangles; i++) {
			...
			if (edgeInfo01 == null) {
				...
				if (edgeInfo01.m_face1 != null) {
					System.out.println("BuildStripifyInfo: > 2 triangles on an edge" + v0 + "," + v1
			...
			if (edgeInfo12 == null) {
				...
				if (edgeInfo12.m_face1 != null) {
					System.out.println("BuildStripifyInfo: > 2 triangles on an edge" + v1 + "," + v2
							+ "... uncertain consequences\n");
				} else {
			...
			if (edgeInfo20 == null) {
				...
				if (edgeInfo20.m_face1 != null) {
					System.out.println("BuildStripifyInfo: > 2 triangles on an edge" + v2 + "," + v0
							+ "... uncertain consequences\n");
				} else {
	...
	static int findStartPoint(FaceInfoVec faceInfos, EdgeInfoVec edgeInfos) {
		...
		for (int i = 0; i < faceInfos.size(); i++) {
			...
			if (findOtherFace(edgeInfos, faceInfos.at(i).m_v0, faceInfos.at(i).m_v1, faceInfos.at(i)) == null)
				ctr++;
			if (findOtherFace(edgeInfos, faceInfos.at(i).m_v1, faceInfos.at(i).m_v2, faceInfos.at(i)) == null)
				ctr++;
			if (findOtherFace(edgeInfos, faceInfos.at(i).m_v2, faceInfos.at(i).m_v0, faceInfos.at(i)) == null)
				ctr++;
			if (ctr > bestCtr) {
				bestCtr = ctr;
				bestIndex = i;
	...
	int createStrips(StripInfoVec allStrips, IntVec stripIndices, boolean bStitchStrips) {
		...
		for (int i = 0; i < nStripCount; i++) {
			...
			{
				FaceInfo tFirstFace = new FaceInfo(strip.m_faces.at(0).m_v0, strip.m_faces.at(0).m_v1,
				...
				if ((i == 0) || !bStitchStrips) {
					if (!isCW(strip.m_faces.at(0), tFirstFace.m_v0, tFirstFace.m_v1))
					...
					if (nextIsCW(stripIndices.size() - accountForNegatives) != isCW(strip.m_faces.at(0),
							tFirstFace.m_v0, tFirstFace.m_v1)) {
						stripIndices.add(tFirstFace.m_v0);
					}
				}
	...
	void findAllStrips(StripInfoVec allStrips, FaceInfoVec allFaceInfos, EdgeInfoVec allEdgeInfos, int numSamples) {
		...
		while (!done) {
			...
			HashSet<FaceInfo> /* NvFaceInfo */
			resetPoints = new HashSet<FaceInfo>();
			for (int i = 0; i < numSamples; i++) {
				...
				StripInfo strip10 = new StripInfo(new StripStartInfo(nextFace, edge10, false), stripId++,
						experimentId++);
				...
				StripInfo strip21 = new StripInfo(new StripStartInfo(nextFace, edge21, false), stripId++,
						experimentId++);
				...
				StripInfo strip02 = new StripInfo(new StripStartInfo(nextFace, edge02, false), stripId++,
						experimentId++);
	...
	//
	void splitUpStripsAndOptimize(StripInfoVec allStrips, StripInfoVec outStrips, EdgeInfoVec edgeInfos,
	...
	//
	void stripify(IntVec in_indices, int in_cacheSize, int in_minStripLength, int maxIndex, StripInfoVec outStrips,
			FaceInfoVec outFaceList) {
		meshJump = 0.0f;
