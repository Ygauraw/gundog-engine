...
public class BottomTowerLayout implements ILayout {
	private Activity activity;
	private ImageView cancelImage;
	private ImageView buyImage;
	private Player player;

	public BottomTowerLayout(Activity activity, final BottomLayout bottomLayout, final CenterLayout centerLayout) {
		...
		buyImage = new ImageView(activity);
		buyImage.setImageBitmap(BitmapCache.getBitmap(R.drawable.bottommenu_buy));
		buyImage.setPadding(padding, 0, padding, 0);
		buyImage.setClickable(true);
		buyImage.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Modules.MESSENGER.submit(LogicMessageProcessor.ID, LogicMessageProcessor.BUILD_ALL_TOWERS,
						player.getID());
				bottomLayout.attachDefaultLayout();
				centerLayout.attachBoardLayout();
			}
	...
	public void setTower(BaseTower tower) {
		...
		image.setImageBitmap(BitmapCache.getBitmap(R.drawable.icon_stat_damage));
		right.addView(image);

		TextView text = new TextView(activity);
		text.setTextSize(textSize);
		text.setPadding(widthPadding, 0, widthPadding, 0);
		text.setText(formatter.format(tower.getDamage()));
		text.setBackgroundColor(backgroundColor);
		right.addView(text);

		image = new ImageView(activity);
		image.setImageBitmap(BitmapCache.getBitmap(R.drawable.icon_stat_attack_rate));
		right.addView(image);

		text = new TextView(activity);
		text.setTextSize(textSize);
		text.setPadding(widthPadding, 0, widthPadding, 0);
		text.setText(formatter.format(tower.getAttackRate()));
		text.setBackgroundColor(backgroundColor);
		right.addView(text);
