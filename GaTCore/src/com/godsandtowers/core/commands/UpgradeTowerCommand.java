...
public class UpgradeTowerCommand implements Runnable {
	private GameInfo stats;
	private Player player;
	private float x;
	private float y;

	public UpgradeTowerCommand(GameInfo stats, Player player, float x, float y) {
		this.stats = stats;
		this.player = player;
		this.x = x;
		this.y = y;
	...
	public void run() {
		...
		if (oldTower != null && player.getGold() >= oldTower.getCost()) {
			player.decreaseGold(oldTower.getCost());

			BaseTower baseTower = player.getUpgrade(oldTower.getBaseTower());
			...
			if (newTower != null) {
				...
				Modules.MESSENGER
						.submit(GLMessageProcessor.ID, GLMessageProcessor.ADD_SPRITE, player.getID(), newTower);
